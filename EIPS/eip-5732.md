...
1. The reference implementations are for demo purpose only. Since EIP only mandates the commit, but not the reveal side, Implementers are recommended to revisit their own use-case carefully to choose reveal approach.
